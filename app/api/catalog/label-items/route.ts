...
//
// 1kg は業販専用で店頭では売らないため Square には登録しない (Rowly 2026-08-18)。
// Square に入れると価格付きの商品としてレジに並んでしまうし、GTIN も業販では
// 不要なので GS1 の番号は消費せず pseudoUpc() の店内用 (200始まり・POSスキャン
// 不可) を使う。ラベルに価格は印字されないので price は 0 でよい。
//
// 対象外にするもの:
//  - ドリップパック (最小サイズが10g以下)。1kgのドリップパックは存在しない
//  - 既に Square 側に 1kg があるもの (FELICITY葉山ブレンド / PNG Baroida)
const WHOLESALE_GRAMS = 1000

// 既存SKU (INA-100, COL-200 …) から接頭辞を取り、1kg 用の SKU を作る。
// 既存の 1kg は FEL-1000 / PNG-1000 なので "-1000" に揃える。
function wholesaleSku(variations: Variation[]): string {
  for (const v of variations) {
    const m = v.sku.match(/^([A-Za-z]+)-\d+/)
    if (m) return `${m[1].toUpperCase()}-1000`
  }
  return ''
}

function addWholesaleKilo(items: LabelItem[]): void {
  for (const item of items) {
    const grams = item.variations.map((v) => sizeToGrams(v.size))
    const sized = grams.filter((g) => g > 0)
    if (sized.length === 0) continue
    if (Math.min(...sized) <= 10) continue          // ドリップパック
    if (grams.some((g) => g >= WHOLESALE_GRAMS)) continue  // 既に1kgがある

    const variationId = `felicity-wholesale-1kg-${item.itemId}`
    item.variations.push({
      variationId,
      sku: wholesaleSku(item.variations),
      upc: pseudoUpc(variationId),
      size: '1kg',
      price: 0,
      type: 'bean',
    })
    item.variations.sort((a, b) => sizeToGrams(a.size) - sizeToGrams(b.size))
  }
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 業販用 1kg を全ての豆に自動追加（ドリップパックと既に1kgがあるものは除く）
    addWholesaleKilo(items)
