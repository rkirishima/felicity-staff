...
function SettingsContent() {
  ...
  async function changePin() {
    ...
    const correct = data?.pin || '1234'
    if (currentPin !== correct) { toast.error('現在のPINが違います'); setLoading(false); return }
  ...
  return (
    <main className="min-h-screen p-4 max-w-lg mx-auto pb-24" style={{ backgroundColor: '#F5F0E8' }}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-lg font-bold tracking-widest text-stone-800">設定</h1>
      </div>

      <div className="bg-white rounded-2xl shadow-sm p-5 space-y-4">
        <p className="text-sm font-medium text-stone-700">🔐 PIN変更</p>
        <div>
